...
urvdb_filename = os.path.join(wdir, 'U-RVDBv' + currentvs, '.fasta')
raw_clustering_infilename = os.path.join(wdir, sys.argv[4])
...
def write_promoted_clusters(raw_clustering_infilename, descs):
    ...
    inf = open(raw_clustering_infilename, "r")
...
def collect_rvdb_descriptions(urvdb_filename):
    ...
    with open(urvdb_filename, "r") as inf:
        for line in inf:
...
def write_crep_outfile(promoted_clustering_filename):
    ...
    inf = open(promoted_clustering_filename, "r")
