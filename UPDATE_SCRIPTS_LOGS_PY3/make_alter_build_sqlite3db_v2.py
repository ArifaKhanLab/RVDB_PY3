...
def create_new_db(dbname, tablenames, tablefields, tabletypes, pkeyindex):
    ...
    print(os.path.join(wdir, dbname))
    conn = sqlite3.connect(os.path.join(wdir, dbname))
    c = conn.cursor()
...
def build_db(rvdb_dir, rvdb_filename):
    ...
    with open(os.path.join(rvdb_dir, rvdb_filename), "r") as inf:
        for line in inf:
            if line.startswith('>acc'):
                if not seqlen == 'NA':
                    c.execute("INSERT INTO " + str(maintable) + " (" + str(field1) + ", " + str(field2) + ", " + str(
                        field3) + ", " + str(field4) + ", " + str(field5) + ", " + str(field6) + ", " + str(
                        field7) + ", " + str(field8)
                              + ") VALUES (?,?,?,?,?,?,?,?)",
                              (acc, source, description, seqlen, organism, division, date, biocategory))
                    c1 += 1
                    if c1 - c2 == 100000:
                        print
                        str(c1) + ' entries from ' + rvdb_filename + ' parsed into sqlite3 format'
                        c2 = c1
                sl = line.strip().split('|')
                source = sl[1]
                acc = sl[2]
...
def query_db(dbname, outfilename, conditions, boolcond):
    ...
    with open(os.path.join(wdir, outfilename), 'w') as outf:
        for row in rows:
            print(row)
            outf.write('|'.join(row) + '\n')
