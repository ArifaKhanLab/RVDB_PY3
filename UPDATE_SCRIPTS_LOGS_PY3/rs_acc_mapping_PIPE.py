...
def get_duplicate_acc(clustering_filename):
    ...
    inf = open(clustering_filename, "r")
    dups = dict()
    mappable = False
...
def get_gb_comments(comments_dict, gb_filename):
    ...
    #### Jaysheel: Keep this as is, give that gb file has data over mulitple lines
    #### list(map(lambda x: x.strip(), inf.readlines())) will not produce the same result as expected.
    #### comments will have incorrect entry.
...
logf = open(os.path.join(logdir, 'mapping_refseq_genbank.txt'), 'w')

for fn in os.listdir(refseqdir):
    if fn.startswith('refseqviral_neighbors_mapping') and fn.endswith('.csv'):
        nmapfilename = os.path.join(refseqdir, fn)
        break

refseq_gbfilenames = []
for fn in os.listdir(refseqdir):
    if fn.startswith('viral.genomic_'):
        refseq_gbfilenames.append(os.path.join(refseqdir, fn))
...
rvdb_rs_accs = getaccs(os.path.join(refseqdir, 'viral.genomic.eukviral.accs.txt'))
...
logf.write(str(len(
    dups2)) + ' refseq viral accessions mapped to original entries using GenBank metadata (comments section)' + '\n')
logf.write(str(len(set(dups.keys()).intersection(
    set(dups2.keys())))) + ' refseq viral accessions in common between the two above' + '\n')
...
for rs_acc in rvdb_rs_accs:
    try:
        ...
        try:
            ...
            continue
logf.write(str(len(dups3)) + ' of ' + str(
    len(rvdb_rs_accs)) + ' refseq viral accessions mapped to original entries in total' + '\n')

outfilename = os.path.join(refseqdir, 'refseq_viral_originalaccs.txt')
outf = open(outfilename, 'w')
outf.write('\n'.join(sorted(dups3.values())))
outf.close()
logf.close()

outfilename = os.path.join(refseqdir, 'neighbor_accs.txt')
print(outfilename)
