...
gbrelfilename = os.path.join(logdir, relnotes)
...
outf = open(os.path.join(logdir, 'RVDBv' + currentvs + '_checkpt2a.log'), 'w')
unzipfns = []
...
for f, fn in enumerate(os.listdir(gbdir)):
    ...
    existing_fnnum = 0
    inf = open(os.path.join(gbdir, fn))
...
outf = open(os.path.join(logdir, currentvs + '_checkpt2b.log'), 'w')
cdict2 = dict()
...
inf = open(os.path.join(logdir, currentvs + '_checkpt2b.log'), "r")
cdict2 = dict()
for i, line in enumerate(inf):
...
outf = open(os.path.join(logdir, currentvs + '_checkpt2c.log'), 'w')
...
outf = open(os.path.join(logdir, currentvs + '_checkpt2d.log'), 'w')
